...
### 2.4 軟ルーター・透明ゲートウェイ環境向け接続プール最適化ガイド (Soft Router Gateway Tuning)

Mihomo (Clash.Meta) を OpenWrt や Linux 透明プロキシ（TPROXY / TUN）などの**軟ルーター・ホームゲートウェイ**環境で運用する場合、LAN 内の複数端末（PC、スマートフォン、IoT、NAS など）から数百〜数千の並行接続が集中します。Chitanda の接続プールパラメーターを最適化することで、回線詰まりやゲームの遅延スパイクを防止できます。

#### ① パラメーター機能と設計原則
- **`pool-size` (TCP 物理接続数)**：クライアントとサーバー間で確立・維持する独立した TLS 1.3 / H2 物理長接続の数（デフォルト: `4`）。Web 閲覧や API、ダウンロードの TCP 多重化ストリームを分散し、単一 TCP のパケットロスによる隊頭閉塞（Head-of-Line Blocking）を分散・緩和します。
- **`udp-pool-size` (UDP 物理接続数)**：独立した H3 / QUIC 物理データグラムキャリアの数（デフォルト: `pool-size` と同一値、通常 `4`）。オンラインゲーム、DNS、Discord などの独立した UDP セッションを物理的に異なる QUIC コネクションへ最小負荷順に割り当て、**大容量 UDP ダウンロードによるゲームの遅延急増（Bufferbloat）を物理的に完全隔離**します。
- **ゼロコンフィグ標準動作**：一般的な PC やスマホの単体利用では、設定ファイル内で `pool-size` や `udp-pool-size` を明示する必要はありません。自動的に `TCP 4 + UDP 4` の堅牢なプールが起動します。

#### ② 軟ルーターハードウェア別推奨設定マトリクス

| ハードウェアタイプ | 典型構成例 | 推奨 `pool-size` (TCP) | 推奨 `udp-pool-size` (UDP) | リソースと動作評価 |
| :--- | :--- | :---: | :---: | :--- |
| **主流 x86 軟ルーター**<br>(Intel N100, N5105, J4125、メモリ $\ge 4\text{GB}$) | ギガビット光回線、LAN 内 10〜50 台並行 | **6 〜 8** | **4** | **最も推奨される黄金設定**。4 コア CPU を均等に活用し、多端末の同時アクセス耐性を最大化。メモリ増加は 40〜60MB 程度で極めて軽微。 |
| **軽量 ARM / エッジルーター**<br>(MediaTek MT7981, 7986, Rockchip RK3568、メモリ 512MB〜1GB) | 一般家庭 5〜10 台端末 | **4** | **2 〜 4** | **デフォルト 4 を維持推奨**。突発的な大量バースト時に UDP リングバッファでメモリが枯渇し、OOM Killer が発動するのを防止。 |
| **産業用・10G 万兆ゲートウェイ**<br>(Intel Core i7/Xeon、2.5G/10G 光回線) | 2.5G/10G 専用線、数百端末の大規模高並行 | **8 〜 12** | **6 〜 8** | 万兆クラスの高速中継をフル活用。超大スループット時もリアルタイム通信の物理分離を維持。 |

> [!WARNING]
> **接続数（pool-size）を 16 以上に無制限に拡大してはならない理由**：
> 1. **公網 Bufferbloat による遅延爆発**：対外的な物理回線帯域には上限があります。物理長接続を過剰に並行確立すると、各接続の輻輳制御ウィンドウ（CWND）がルーターの限られたキューで激しく衝突（Congestion Thrashing）し、かえって Ping が 40ms から 500ms 以上に跳ね上がり、大量のパケットロスを自発的に引き起こします。
> 2. **DPI 異常検知リスク**：単一クライアントから同一の海外単一 IP/ポートに対して数十本ものアクティブな TLS/QUIC 長接続を維持する動作は、ISP のトラフィック監査において典型的な機械的異常とみなされ、QoS 帯域制限の対象となりやすくなります。

#### ③ 軟ルーターゲートウェイ向け設定サンプル (`config.yaml`)
```yaml
proxies:
  - name: "Chitanda-Tokyo-Gateway"
    type: chitanda
    server: jp.example.com
    port: 443
    ...
    transport: "h2"
    sni: "jp.example.com"
    pool-size: 8          # 軟ルーター配下の高並行 TCP 向け (8 本の H2 パイプ)
    udp-pool-size: 4      # ゲーム・DNS などの QoS 物理分離向け (4 本の独立 H3 パイプ)
    udp: true
```
