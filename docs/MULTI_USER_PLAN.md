...
1. **Stream 握手试算**：
   原实现的 30 用户最坏位置基准在 Windows/amd64 为 56.7 µs/op。通过在服务端启动时预计算每个用户的掩码，生产路径基准降至 **19.8 µs/op、145 allocs/op**，达成本机 `<25 µs` 目标。
2. **UDP 路径解耦优化（方案 A 落地）**：
   此前 `auto` 入站同端口混跑 H3 与 Native Plain-UDP 时，非 Plain-UDP 报文对 1/30/128 用户需要 1.5/45/215 µs/包的试解密惩罚。经工程评估与用户确认，生产环境（如 3X-UI / Xray 标准实践）入站端口均有独立确定的协议，不存在同端口混跑需求。已将入站 UDP 路径按 Transport 分立：
   - `h2` / `h3` / `auto`：UDP 路径专用于 HTTP/3 (QUIC Datagrams)，直接投递 QUIC 栈，**Plain-UDP 试解密开销彻底归零（0 µs）**。
   - `stream` / `h1` / `plain-h1`：UDP 路径专用于 Native Plain-UDP，仅在此类入站中按多用户循环解密。
   这彻底消除了多用户下 H3 的 UDP 试解密瓶颈。仍须在目标生产机测有效密钥位置、真实吞吐与尾延迟。
