...
认证与解密试算的 CPU 开销随用户数增长；尤其是 `auto` 的 UDP 入口会在判断 H3 报文前尝试原生 UDP 解密。本机 Windows/Hygon 对 1200B 非 Plain-UDP 报文实测 1/30/128 用户约 1.5/45/215 µs/包，仅作当前实现的微基准；仍须在目标机器测有效密钥位置、真实 H3 流量、吞吐与尾延迟。优化此路径不得默默破坏同端口 H3/Plain-UDP 的既有混合行为。
