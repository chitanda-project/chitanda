...
- 注入 Xray 后 `go test -mod=mod ./proxy/chitanda -count=1 -timeout=180s`：通过。
...
2. H3/auto **双用户并发端到端**及真实 Xray Policy 计费尚无完整覆盖。现有测试分别覆盖通用 HTTP 身份、单用户 H3 路径和 Xray Stats 机制，但组合风险仍在。
