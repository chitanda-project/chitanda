...
> 状态（审计中）：以下 Proposed Changes 保留原始施工计划，并非已验收事实。当前代码已覆盖多用户身份与原生 UDP；H3/auto 已有两用户分别连通回归，但并发计费尚未验收。30 用户最坏位置握手匹配经启动时预计算优化，在 Windows/Hygon 上由约 56.7 µs 降为 19.8 µs（原目标 <25 µs）；Linux race 已通过，但 CI 构建步骤重跑与目标机器性能仍待验证。详见 `docs/MULTI_USER_AUDIT.md`。
